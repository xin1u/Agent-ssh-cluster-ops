...
description: Operate explicitly configured shared SSH hosts for bounded GPU, Git, process, and tmux audits; reviewed local-to-remote Git diff application; exact tree verification; persistent OpenSSH control sockets; and exact managed tmux sessions. Use from any Agent Skills-compatible coding agent. Do not use for arbitrary shell access, secrets, data transfer, scheduler changes, or broad process control.
license: Apache-2.0
metadata:
  author: xin1u
  version: "0.3.0"
...
1. Treat the JSON policy as a technical allowlist, not permission for an unrelated action. Respect the user's requested scope and the cluster's operational rules.
...
The CLI intentionally has no `exec`, interactive shell, scheduler, upload, download, copy, delete, reset, clean, commit, or push subcommand. Use established source-publication and approved data-transfer workflows separately.
