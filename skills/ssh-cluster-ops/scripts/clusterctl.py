...
DEFAULT_POLICY_PATH = "~/.config/ssh-cluster-ops/policy.json"
LEGACY_DEFAULT_POLICY_PATH = "~/.config/codex-ssh-cluster-ops/policy.json"
...
def load_policy(path_value: str) -> Policy:
    ...
    if settings.audit_log.name != "audit.jsonl" or settings.audit_log.parent.name not in {
        "ssh-cluster-ops",
        "codex-ssh-cluster-ops",
    }:
        raise ClusterError(
            "settings.audit_log must end in ssh-cluster-ops/audit.jsonl or "
            "codex-ssh-cluster-ops/audit.jsonl"
        )
...
def _temporary_index_tree_and_diff(repo: Path, expected_head: str) -> Tuple[str, bytes]:
    with tempfile.TemporaryDirectory(prefix="sshops-") as directory:
...
def _load_git_diff(path_value: str, limit: int) -> bytes:
    ...
    if b"*** Begin Patch" in data or b"*** Update File:" in data:
        raise ClusterError("non-Git patch grammar is not accepted; use clusterctl make-diff")
...
def command_make_diff(policy: Policy, args: argparse.Namespace) -> None:
    ...
    with tempfile.NamedTemporaryFile(prefix="sshops-review-", delete=False) as temporary:
...
def _heredoc(payload: bytes, stem: str) -> Tuple[str, str]:
    ...
    while True:
        delimiter = f"SSHOPS_{stem.upper()}_{secrets.token_hex(16)}"
...
patch_file=$(mktemp "$git_dir/sshops-patch.XXXXXX")
index_file=$(mktemp "$git_dir/sshops-index.XXXXXX")
rm -f -- "$index_file"
...
head=$(git -C "$repo" rev-parse HEAD)
[[ "$head" == "$expected_head" ]] || { printf 'HEAD mismatch: %s != %s\n' "$head" "$expected_head" >&2; exit 75; }
snapshot_index=$(mktemp "$git_dir/sshops-snapshot.XXXXXX")
rm -f -- "$snapshot_index"
...
tmux set-option -t "=$name" @sshops_run_dir "$run_dir"
tmux set-option -t "=$name" @sshops_expected_head "$expected_head"
tmux set-option -t "=$name" @sshops_expected_tree "$expected_tree"
...
run_dir=$(tmux show-options -qv -t "=$name" @sshops_run_dir)
if [[ -z "$run_dir" ]]; then
...
expected_head=$(tmux show-options -qv -t "=$name" @sshops_expected_head)
if [[ -z "$expected_head" ]]; then
  expected_head=$(tmux show-options -qv -t "=$name" @codex_expected_head)
fi
expected_tree=$(tmux show-options -qv -t "=$name" @sshops_expected_tree)
if [[ -z "$expected_tree" ]]; then
  expected_tree=$(tmux show-options -qv -t "=$name" @codex_expected_tree)
fi
printf 'session=%s\nstate=present\n' "$name"
printf 'run_dir=%s\n' "$run_dir"
...
def _default_policy_path(environment: Optional[Mapping[str, str]] = None) -> str:
    environment = os.environ if environment is None else environment
    for variable in ("SSH_CLUSTER_OPS_POLICY", "CODEX_SSH_CLUSTER_POLICY"):
        configured = environment.get(variable)
        if configured:
            return configured
    if Path(os.path.expanduser(DEFAULT_POLICY_PATH)).exists():
        return DEFAULT_POLICY_PATH
    if Path(os.path.expanduser(LEGACY_DEFAULT_POLICY_PATH)).exists():
        return LEGACY_DEFAULT_POLICY_PATH
    return DEFAULT_POLICY_PATH
